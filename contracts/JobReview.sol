...
contract JobReview {
    ...
    function createJobListing(
        ...
        emit JobListingCreated(_companyName, _jobTitle, _desc);
    }
    ...
    function updateJobListing(
        ...
        // Update the JobListing struct with new values
        jobListings[_id].companyName = _companyName;
        jobListings[_id].jobTitle = _jobTitle;
        jobListings[_id].description = _desc;
    }

    function deleteJobListing(uint _id) public {
        require(_id < jobListings.length, "JobListing does not exist");

        //emit LogMessage("beforeFOR");
        // Delete all reviews associated with the job listing
        for (uint i = reviews.length; i > 0; i--) {
            //emit LogMessage("inFOR");
            if (reviews[i - 1].jobId == _id) {
                //emit LogMessage("inIF");
                deleteReview(i - 1);
            }
        }

        // Move the last JobListing to the position of the JobListing to be deleted
    ...
    function getCompanySalaryStats(
        ...
        require(numReviews > 0, "No reviews found for the given company");
    ...
    function getJobTitleSalaryStats(
        ...
        require(numReviews > 0, "No reviews found for the given job title");
        uint avgSalary = totalSalary / numReviews;
        return (avgSalary, minSalary, maxSalary);
    }
    ...
    function getJobTitleSalaryStatsAtCompany(
        ...
        require(
            numReviews > 0,
            "No reviews found for the given job title and company"
        );
        uint avgSalary = totalSalary / numReviews;
        return (avgSalary, minSalary, maxSalary);
    }
